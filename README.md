...
Claude Desktop has no built-in profile switcher — if you have a personal and a work account, you're stuck logging out and back in. Switch Claude fixes that: each account keeps its own Claude data directory, so **every account stays signed in permanently**, and switching takes one keystroke.
...
```sh
git clone https://github.com/kevinjdolan/switch-claude.git
cd switch-claude
./build-app.sh
...
Claude Desktop is an Electron app, so it honors Chromium's `--user-data-dir` flag. Each account you add gets its own data directory under `~/Library/Application Support/SwitchClaude/Profiles/<name>`; the Default account uses Claude's standard directory. Switching is just: quit Claude → relaunch it with `--user-data-dir` pointed at the selected profile. Settings live in `~/Library/Application Support/SwitchClaude/accounts.json`.
...
```sh
open -n -a Claude --args --user-data-dir="$HOME/Library/Application Support/SwitchClaude/Profiles/<name>"
```

## Notes

- If Claude refuses to quit (e.g. it's showing a modal dialog), the switch aborts with an error rather than force-killing it.
- The hotkey uses Carbon's `RegisterEventHotKey` — no accessibility permission needed. To rebind it, edit the key code/modifiers in `Sources/SwitchClaude/HotKeyManager.swift` and rebuild.
- **Start at Login** registers the app at its current path; if you move the app, toggle it off and on again.
- Uninstall: quit from the menu bar, delete `/Applications/Switch Claude.app`, and optionally remove `~/Library/Application Support/SwitchClaude` (this deletes the extra account profiles and their logins).
